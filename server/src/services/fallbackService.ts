...
export class FallbackService {
  public static generateSummaryFallback(
    items: ApprovalItem[] = MOCK_APPROVALS_QUEUE,
    reason = "Deterministic fallback generated due to AI service timeout or unconfigured LLM API."
  ): SummaryOutputType {
    ...
    const mappedItems = sorted.map((item) => {
      let recommendedAction: "APPROVE" | "REJECT" | "REQUEST_CHANGES" | "ESCALATE" = "APPROVE";
  ...
  public static generateGreetingFallback(
    operatorName = "Operator",
    items: ApprovalItem[] = MOCK_APPROVALS_QUEUE,
    reason = "Generated via rule template"
  ): GreetingOutputType {
  ...
  public static generateHelpFallback(
    question: string,
    reason = "Deterministic SOP policy retrieval"
  ): HelpOutputType {
  ...
  public static generateTeachFallback(
    step = 1,
    targetItem?: ApprovalItem,
    reason = "Deterministic SOP step engine"
  ): TeachOutputType {
  ...
  public static generateTalkResponse(userQuestion: string): string {
