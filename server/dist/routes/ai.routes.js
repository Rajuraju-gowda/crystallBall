...
import { SummaryRequestSchema, HelpRequestSchema, TeachRequestSchema, GreetingRequestSchema, TalkRequestSchema } from "../schemas/aiRequest.schema.js";
import { SummaryOutputSchema, HelpOutputSchema, TeachOutputSchema, GreetingOutputSchema } from "../schemas/aiResponse.schema.js";
import { SUMMARY_PROMPT_V1, GREETING_PROMPT_V1, HELP_PROMPT_V1, TEACH_PROMPT_V1, TALK_PROMPT_V1 } from "../prompts/index.js";
...
aiRouter.post("/summary", validateRequestBody(SummaryRequestSchema), async (req, res) => {
    const { language, focusArea } = req.body;
    const result = await withAIFallback(async (signal) => {
        const userPrompt = SUMMARY_PROMPT_V1.buildUserPrompt({
            queueItems: MOCK_APPROVALS_QUEUE,
            language,
            focusArea
        });
        const rawOutput = await aiClient.generateText(SUMMARY_PROMPT_V1.systemPrompt, userPrompt, signal);
...
aiRouter.post("/greeting", validateRequestBody(GreetingRequestSchema), async (req, res) => {
    ...
    const result = await withAIFallback(async (signal) => {
        ...
        const rawOutput = await aiClient.generateText(GREETING_PROMPT_V1.systemPrompt, userPrompt, signal);
        const cleaned = rawOutput.replace(/```json\n?|\n?```/g, "").trim();
        const parsed = JSON.parse(cleaned);
        return GreetingOutputSchema.parse(parsed);
    }, (fallbackReason) => {
        return FallbackService.generateGreetingFallback(operatorName, MOCK_APPROVALS_QUEUE, fallbackReason);
    }, 8000);
    res.json(result.data);
});
aiRouter.post("/help", validateRequestBody(HelpRequestSchema), async (req, res) => {
    ...
    const result = await withAIFallback(async (signal) => {
        ...
        const rawOutput = await aiClient.generateText(HELP_PROMPT_V1.systemPrompt, userPrompt, signal);
        const cleaned = rawOutput.replace(/```json\n?|\n?```/g, "").trim();
        const parsed = JSON.parse(cleaned);
...
aiRouter.post("/teach", validateRequestBody(TeachRequestSchema), async (req, res) => {
    ...
    const result = await withAIFallback(async (signal) => {
        ...
        const rawOutput = await aiClient.generateText(TEACH_PROMPT_V1.systemPrompt, userPrompt, signal);
        const cleaned = rawOutput.replace(/```json\n?|\n?```/g, "").trim();
        const parsed = JSON.parse(cleaned);
...
aiRouter.post("/talk", validateRequestBody(TalkRequestSchema), async (req, res) => {
